...
*@version 0.1 First ever release lol
...
class Pagination{
	...
	*/
	...
	/**
	*This is the main method, it prints out the links that send the offset variable to .
	*@return string
	...
	public function printNavBar(){
		...
		for($i=$currentPage; $i < ceil( ($currentPage + $this->limit_per_page) + 1);  $i++){
			//print out the nav bar
				...
				//style the current page. You can edit it to 
				if($current === $offset){
					
					$style='color:rgb(140,140,140); font-weight:bold; font-style:italic;';
				}else{
					$style=" ";
				}
	...
	*/
	public function paginate(){
		...
		$offset=(isset($_GET['offset'])) ? $_GET['offset'] : 0;
		$sql= " SELECT * FROM " . $this->database_table_name . " LIMIT " . $this->limit_per_page . " OFFSET ". $offset ;
		$prepared=$this->db->prepare($sql);
		$prepared->execute();
